...
namespace App_M5 {
    public partial class club : Form {
        ...
        private void insertBtn_Click(object sender, EventArgs e) {
            string[] split = tahunDtp.Text.Split(' ');
            string date = split[2];

            if (!edit) {
                if (ligaCb.SelectedValue != null) {
                    string insert = "INSERT INTO club VALUES ('" + idTb.Text + "', '" + namaTb.Text + "', TO_DATE('" + date + "', 'YYYY'), " +
        ...
        private void updateBtn_Click(object sender, EventArgs e) {
            ...
            if (edit) {
                string update = "UPDATE club " +
                                "SET nama_club = '" + namaTb.Text + "', tahun_berdiri = TO_DATE('" + tahun + "', 'YYYY'), " +
